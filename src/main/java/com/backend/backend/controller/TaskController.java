...
//    @PreAuthorize("hasAuthority('Admin')")
	...
	public ResponseEntity<?> assigneeTask(@PathVariable Long id, @RequestBody Map<String, Long> requestBody) {
	...
	public ResponseEntity<List<TaskDto>> getAllTasks() {
		...
		List<TaskDto> dtoList = tasks.stream().map(task -> new TaskDto(task.getId(), task.getTitle(),
				task.getDescription(), task.getStatus(), task.getDueDate(), task.getPriority(),
				(task.getCreator() != null) ? task.getCreator().getFirstName() + " " + task.getCreator().getLastName()
						: null,
	...
	public ResponseEntity<?> completeTask(
	    ...
	    try {
	        ...
	        if (remark == null || remark.trim().isEmpty()) {
	            return ResponseEntity.badRequest().body("Missing 'remark' in request body");
	        }

	        taskService.completeTask(id, remark);
